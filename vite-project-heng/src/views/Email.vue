...
// 发送邮件
const sendEmail = async ({ content, contentBlock,recipient }) => {
  try {
    // 构造发送邮件的数据
    const emailData = {
  ...
  <div class="h-screen flex flex-col">
    ...
    <div class="contain bg-myBgColor flex flex-1 rounded-md">
      ...
      <div class="calc-h flex-1 ml-0 m-3 bg-white flex">
        <SendEmail :users="users" :menuActive="menuActive" @sendEmail="sendEmail" v-if="menuActive === 1" />
        <div class="w-full bg-slate-500" v-else-if="menuActive === 2">发邮件</div>
        <ReceivedEmail :email="userInfo.email" :messages="messages" v-else-if="menuActive === 3" />
        <div class="w-full bg-slate-800" v-else>草稿</div>
      </div>
    </div>
  </div>
</template>
