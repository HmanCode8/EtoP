...
rl.question('请输入需要生成的模版json文件：', (jsonName) => {
  fs.readFile(path.join(__dirname, `/${jsonName}.json`), 'utf-8', (err, data) => {
    if (err) {
      ...
      rl.question('请输入请求函数文件名（不含扩展名）：', (name) => {
        ...
        keys.forEach((k) => {
          const { url = 'test', method = 'test', description = '', body = {} } = mks[k]
        ...
        name = name.includes('.') ? name : `${name}.ts`

        // 将请求函数代码和导出代码写入文件
        const fileName = path.join(__dirname, `/request/${name}`)
