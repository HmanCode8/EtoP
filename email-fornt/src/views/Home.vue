...
<template>
  <div class="email-car-bg-color fixed top-0 left-0 w-full h-16 pl-10 pr-10 z-20 flex justify-between items-center">
    <div class="flex items-center">
      <a href="http://www.shihenghe.online:90/" class="logo flex items-center">
        <div class="text-5xl font-bold">HSH</div>
        <span class="text-sm">blog</span>
      </a>
      <div ref="navRef" class="nav-list flex">
        <el-popover placement="top-start" :width="200" trigger="hover" :content="`技术凸显：${item.desc}`"
          v-for="item in navList" :key="item.navName">
          <template #reference>
            <div @click="onNavClick(item.navPath)" :class="[
              ...
              {{ item.navName }}
              <div :class="`nav-bar w-13 h-0.5 rounded-br-xl rounded-bl-xl scale-0 group-hover:scale-100 ${userStore.navActive === item.navPath ? 'active' : ''
                }`"></div>
            </div>
          </template>
        </el-popover>
  ...
  <div class="email-car-bg-color shadow-xl rounded-md clear-both p-5 mx-10 my-10 home-footer py-5 text-center">
    <!-- 行高：1.5rem; -->
    ...
    <div class="icon-list flex justify-center items-center my-5">
      ...
      <a class="icon-item inline-block w-8 h-8 mr-5" title="添加群聊" target="_blank" rel="nofollow"
        href="https://qm.qq.com/q/tX8dU3hBu0"></a>
      <a class="icon-item inline-block w-8 h-8 mr-5" title="Github" target="_blank" rel="nofollow"
        href="https://github.com/HmanCode8"></a>
