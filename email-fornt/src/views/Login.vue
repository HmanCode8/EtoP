    ...
    <div class="login-modal shadow-2xl rounded-lg w-1/2 h-1/2 flex email-box-shadow overflow-hidden">
      ...
      <div class="m-r w-2/3 flex flex-col justify-center items-center">
        ...
        <div class="forms w-2/3 p-3">
          <form action="#" @submit.prevent="isRegister ? onRegister() : onchageLogin()">
            <div class="username flex items-center">
              <input class="email-form-item w-full" v-model="form.username" placeholder="username" />
              <div class="prixt emial-prefix">@hsh.com</div>
            </div>
            <div class="password flex relative">
              <!-- 可显示可隐藏密码 -->
              <input class="email-form-item w-full" v-model="form.password" placeholder="password" :type="isShowPasswork ? 'text' : 'password'" />
