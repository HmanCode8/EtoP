...
<template>
  ...
  <div v-if="wallpaperList.length > 0" class="wallpaper-list relative flex flex-wrap mt-5">
    <div :class="`wallpaper-${index % 3 === 0 ? 'item' : 'item-reverse'} group bg-white relative hover:cursor-pointer rounded-lg hover:shadow-lg`" href="" v-for="w,index in wallpaperList" :key="w.id">
  ...
  <el-dialog v-model="dialogVisible" title="图片详情" width="60%" draggable>
    <div class="flex">
      <img class="shadow-lg w-2/3 rounded-lg" :src="imgDetail.largeImageURL" alt="" srcset="" />
      <div class="detail mr-auto ml-4 leading-8">
        <div class="flex justify-between items-center mb-2">
          作者：<img :src="imgDetail.userImageURL" class="w-10 h-10 rounded-full" alt="" />
        ...
        <div>大小：{{ (Number(imgDetail.imageSize) / 1024 / 1024).toFixed(2) }}MB</div>
...
.wallpaper-list {
  --theme: #9999ff;
  .wallpaper-item,.wallpaper-item-reverse {
    --n: 4 /* 图片数量 */;
    --w: 300px;
    --h: 200px;
