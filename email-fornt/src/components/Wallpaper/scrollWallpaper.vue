...
<template>
  ...
  <ImageViewer
    ...
    @nextPage="onNextPage"
    v-if="dialogVisible"
    :url="imgDetail.largeImageURL"
    :images="puts.images"
  />
  <div
    v-if="wallpaperList.length > 0"
    class="wallpaper-list relative flex justify-between flex-wrap mt-5"
  >
    <div
      :class="`wallpaper-${
        index % 3 === 0 ? 'item' : 'item-reverse'
      } group bg-white relative hover:cursor-pointer rounded-lg hover:shadow-lg`"
      href=""
