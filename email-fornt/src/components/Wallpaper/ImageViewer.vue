...
<template>
  <div class="image-viewer relative h-full">
    <div class="h-full w-full flex flex-col justify-center items-center">
      <!-- 顺序播放 -->
      <div>
        <div @click="onPlay" class="bg-slate-200 flex justify-center items-center absolute top-10 left-10">播放</div>
      </div>
      <div @click="onClose" class="rounded-full text-3xl hover:cursor-pointer absolute top-10 right-10">
        <el-icon><CircleClose /></el-icon>
      </div>
      <div @click="onPreOnchage" class="pre text-3xl hover:cursor-pointer absolute left-10">
        <el-icon><ArrowLeftBold /></el-icon>
      </div>
      <div @click.stop="onNextOnchage" class="next text-3xl hover:cursor-pointer absolute hover:scale-150 right-10">
        <el-icon><ArrowRightBold /></el-icon>
      </div>

      <div ref="imgView" class="viewer-contain h-96 flex justify-center duration-300 items-center">
        <img class="w-full h-full object-cover" :src="currentUrl" alt="" srcset="" />
      </div>
      ...
      <div class="img-list flex mt-4 absolute bottom-10">
        <div
          :class="`${url === currentUrl ? ' scale-150 border-2 border-blue-500' : ''} w-20 h-20 img-item mx-1 hover:cursor-pointer hover:scale-125 duration-300`"
          @click="showImg(index)"
