...
const openUrl = (router: string) => {
  const url = router.includes('http') ? router : 'https://api.oioweb.cn/doc/' + router
  // 检查是否已经存在旧窗口
  ...
  // 打开新窗口
  newWindow.value = window.open(url, '_blank', 'width=800,height=600,toolbar=no,location=no') as any
}
...
<template>
  <div class="apis-header m-5">
    ...
    <h2 class="text-lg mt-2">致力于为开发者提供便捷、免费、稳定、快速的免费Web API数据接口服务。</h2>
    <!-- <div style="font-size: 12px">转载自：（韩小韩的个人博客）</div> -->
    <input v-model="searchVal" type="text" class="email-form-item mt-5 w-full" placeholder="serach by name or description" />
  </div>
