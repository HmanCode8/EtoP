...
function verifyCode(req, res, next) {
  ...
  const key = codeType === 'captcha' ? `captcha_${req.sessionID}` : `verification_code:${phone}`
...
router.post('/login', verifyCode, async (req, res) => {
  try {
    // 从请求体中获取用户提供的信息
    const { username = '', password = '', phone = '', codeType = 'captcha' } = req.body
    let dataBaseConfig = {}
    if (codeType === 'sms') {
      dataBaseConfig = {
        $and: [{ phone }],
    ...
    const { username: name = '', email: em = '', id = '', password: paswod = '', phone: ph = '' } = existingUser

    // 用户验证成功，生成 Token
    const token = jwt.sign({ username: name, email: em, password: paswod, userId: id }, tokenSecret, { expiresIn: '3h' })
    // res.json({ token });
    ...
    //统计登录次数
    const loginCount = await LoginCount.findOne({ username: name })
    if (loginCount) {
      loginCount.count++
      await loginCount.save()
    } else {
      const newLoginCount = new LoginCount({ username: name, count: 1 })
...
router.post('/addPhone', verifyCode, async (req, res) => {
  try {
    ...
    if (existingUser) {
      return res.error({ message: '此手机已经绑定用户' })
    }

    // 更新用户信息
    const updatedUser = await Register.findByIdAndUpdate(userId, { $set: { phone } }, { new: true })
...
//通过手机验证码更新密码
router.post('/updatePassword', verifyCode, async (req, res) => {
  try {
    let { phone, password } = req.body
    // 检查手机号是否存在
    const existingUser = await Register.findOne({ phone })
    if (!existingUser) {
      return res.error({ message: '手机号不存在' })
    }
    // 更新密码
    const updatedUser = await Register.findByIdAndUpdate(existingUser.id, { $set: { password } }, { new: true })
    res.success(updatedUser, '更新成功')
  } catch (error) {
    res.error({ message: error + '更新失败，请稍后重试' })
  }
