...
router.post('/login', verifyCode, async (req, res) => {
  try {
    // 从请求体中获取用户提供的信息
    const { username, password, phone, codeType } = req.body
    let dataBaseConfig = {}
    if (codeType === 'sms') {
      dataBaseConfig = {
        $and: [{ phone }],
      ...
      if (!usernameRegex.test(username)) {
        return res.error({
          message: '用户名格式不正确：用户名：包括字母、数字和下划线，长度在4到16个字符之间。',
        })
      }
      dataBaseConfig = {
    ...
    if (!existingUser) {
      const message = codeType === 'sms' ? '手机号或验证码错误' : '用户名或密码错误'
      return res.notFound({ statusCode: 401, message })
    }
...
router.post('/sendSmsCode', (req, res) => {
  ...
  redisClient.setex(`verification_code:${phone}`, 60, code)
